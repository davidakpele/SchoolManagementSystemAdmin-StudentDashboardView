					...
					<div class="content container content-panel" style="max-width:900px">
						<div class="row">
							<div class="panel panel-primary" id="panel_header">
								<div class="panel-heading text-center"><?= (!empty($data['data']['student_name'])) ? $data['data']['student_name']->Surname.' '.$data['data']['student_name']->othername : '' ;?>&nbsp;Attendance</div>
								<div class="panel-body">
									<div class="table-responsive" style="overflow-x:auto;">
                                        <table class="table table-hover table-bordered" border="1" width="100%">
											...
											foreach ($data['data']['result'] as $key):
											$sn++;
											?>
											<tr>
												<td><?=$sn?></td>
												<td><?=$key['semester'].'00 Level'?></td>
												<td><?=$key['Title']?></td>
												<td><?=$key['CourseCode']?></td>
												<td>&nbsp;<?=(($key['type']==1)?'<span class="badge badge-success bg-gradient-success rounded-pill px-3"><i class="fa fa fa-check">&nbsp;</i>Present</span>':'<span class="badge badge-success bg-gradient-danger rounded-pill px-3"><i class="fa fa fa-close">&nbsp;</i>Absent</span>')?>&nbsp; on <span><?=print_date_on_j($key['doc'])?></span></td>
											</tr>
											<?php endforeach; ?>
											<?php else:?>
											<tr>
