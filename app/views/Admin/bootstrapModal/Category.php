<?php  if(!isLoggedInAdmin()){header('location:' . ROOT . 'Administration/Default');} ?>
<!DOCTYPE html>
<html lang="en">
<!-- BEGIN HEAD -->
<head>
	...
	<link href="<?=ASSETS?>assets/plugins/datatables/plugins/bootstrap/dataTables.bootstrap4.min.css" rel="stylesheet" type="text/css" />
...
<body
					...
					<ul class="nav navbar-nav pull-right">
						...
						<li class="dropdown dropdown-user">
							<a class="dropdown-toggle" data-bs-toggle="dropdown" data-hover="dropdown" data-close-others="true">
								<img alt="" class="img-circle " src="<?=ASSETS?>assets/img/dp.jpg" />
								<span class="username username-hide-on-mobile"> <?=((isLoggedInAdmin())?'Administrator': 'No user');?> </span>
							...
							<ul class="dropdown-menu dropdown-menu-default">
							 <?php if(!isLoggedInAdmin()): ?>
								...
								<?php else: ?>
								<li>
									<a href="<?=ROOT?>Admin/Updatepassword/">
										<i class="icon-user"></i> Change Password </a>
								</li>
								<li>
									<a href="<?=ROOT?>Admin/Change_password/">
				...
				<div class="sidemenu-container navbar-collapse collapse fixed-menu">
					<div id="remove-scroll" class="left-sidemenu">
						<ul class="sidemenu  page-header-fixed slimscroll-style" data-keep-expanded="false"
							...
							<li class="sidebar-user-panel">
								<div class="user-panel">
									<div class="pull-left image">
										<img src="<?=ASSETS?>assets/img/dp.jpg" class="img-circle user-img-circle"
											alt="User Image" />
									</div>
									<div class="pull-left info">
										<p><?=(isset($_SESSION['username'])?$_SESSION['username']: '');?> </p>
										<a href="#"><i class="fa fa-circle user-online"></i><span class="txtOnline">
										<?=(isset($_SESSION['username'])? 'Online': 'Offline');?> </span></a>
									</div>
								</div>
							</li>
							<li class="nav-item start active open">
								<a href="<?=ROOT?>Admin/" class="nav-link nav-toggle">
							...
							<li class="nav-item">
								<a href="<?=ROOT?>Admin/Event/" class="nav-link nav-toggle"> <i class="material-icons">event</i>
							...
							<li class="nav-item">
								...
								<ul class="sub-menu">
									<li class="nav-item">
										<a href="<?=ROOT?>Admin/Professors/" class="nav-link "> <span class="title">All
												Professors</span>
										</a>
									</li>
									<li class="nav-item">
							...
							<li class="nav-item">
								<a href="#" class="nav-link nav-toggle"> <i class="material-icons">person</i>
									<span class="title">Accountant</span> <span class="arrow"></span>
								</a>
								<ul class="sub-menu">
									<li class="nav-item">
										<a href="<?=ROOT?>Admin/Accountant/" class="nav-link "> 
										<span class="title">View Accountant</span>
										</a>
									</li>
								</ul>
							</li>
								...
								<ul class="sub-menu">
									<li class="nav-item active">
										<a href="<?=ROOT?>Admin/Students/" class="nav-link "> <span class="title">All
												Students</span>
											<span class="selected"></span>
										</a>
									</li>
							...
							<li class="nav-item">
								<a href="widget.php" class="nav-link nav-toggle"> <i class="material-icons">widgets</i>
				...
				<div class="page-content">
					<div class="page-bar">
						<div class="page-title-breadcrumb">
							<div class=" pull-left">
								<div class="page-title">Mercy College University Administration Dashboard</div>
							</div>
							<ol class="breadcrumb page-breadcrumb pull-right">
								<li><i class="fa fa-home"></i>&nbsp;<a class="parent-item"
										href="<?=ROOT?>Admin/">Home</a>&nbsp;<i class="fa fa-angle-right"></i>
								</li>
                    ...
                    <div class="container-fluid">
                        <div class="row">
                            <div class="col-md-12">
							<h2 class="text-center">Category & Faculty Table</h2><hr color="black"/>
							<div class="clearfix"></div>
							<button class="btn btn-success" type="button">Create New Category</button>
							<div class="clearfix"></div>
								<div class="col-md-4">
									<h1><?=((isset($_GET['Edit'])? 'Edit Category & Faculty':''))?></h1>
									<!-- Modal -->
												...
												<div class="modal-body">
													...
													<form role="form" method="POST" action="javascript:void(0)" id="ajax-form">
														<div class="form-group">
															...
															<select name="NewParent" class="form-control" id="NewParent">
																<option value="" selected="selected" disabled="disabled">-Empty-</option>
																<?php foreach ($data['catch'] as $key): ?>
																<option value="<<?=$key['Category__ID']?>"><?=$key['Category__name']?></option>
																<?php endforeach; ?>
															</select>
														</div>
														<div class="form-group">
															<label for="ForFaculty">Faculty</label>
															<input type="text" name="NewFaculty" class="form-control" id="NewFaculty" 	value="">
														</div>
													<!-- Modal Footer -->
								...
								<div class="col-md-12">
                                <table class="table table-striped table-bordered table-auto" id="example4" style="max-width:990px">
                                    <thead>
                                        <tr>
                                            <th>ID</th>
                                    ...
                                    <tbody>
									     <?php 
                                            foreach ($data['catch'] as $key): ?>
										
                                        <tr style="background-color:#6777EF; color:#fff">
                                            <td><?=$key['Category__ID']?></td>
                                            <td><?=$key['Category__name']?></td>
                                            <td><?=$key['Parent']?></td>
                                            <td style="display:flex">
												<button type="button" class="btn btn-primary btn-xs" id="<?=$key['Category__ID']?>" data-toggle="modal" data-target="#modalForm" data-parentID="<?=$key['Category__ID']?>">
												<i class="fa fa-pencil"></i>
												</button>
												<?php $BaseId = $key['Category__ID']; ?>
												<button onclick="___Callme(<?=$BaseId?>);" data-id="<?=$BaseId?>" class="btn btn-default btn-xs" id="delete"><i class="fa fa-trash-o"></i></button>
                                            </td>
                                        </tr>
										<?php
										$this->DB = new Database;
										$id = (int)$key['Parent'];
											$this->DB->query('SELECT * FROM `faculty__tb` WHERE Parent__ID = :id');
											$this->DB->bind(':id', $id);
											$st = $this->DB->resultSet();
											foreach ($st as $value) : ?>
										<tr class="bg-light-blue">
											<td><?=$value['Faculty__name']?></td>
											<td><?=$key['Category__name']?></td>
											<td><?=$value['Parent__ID']?></td>
											<td style="display:flex">
											<?php $ROUTE = $value['Faculty__ID']; ?>
                                                <a href="<?=ROOT?>Admin/Category?Edit=<?=$value['Faculty__ID']?>" class="btn btn-primary btn-xs"><i class="fa fa-pencil"></i></a>	
												<button onclick="___CallBack(<?=$ROUTE?>);" data-id="<?=$ROUTE?>" class="btn btn-danger btn-xs" id="delete"><i class="fa fa-remove"></i></button>
                                            </td>
										</tr>
										<?php endforeach?>
                                        <?php endforeach?>
							...
							<div class="chat-sidebar-list">
								<div class="chat-sidebar-chat-users slimscroll-style" data-rail-color="#ddd"
									...
									<ul class="media-list list-items">
										<li class="media"><img class="media-object" src="<?=ASSETS?>assets/img/prof/prof3.jpg"
										...
										<li class="media">
											...
											</div> <img class="media-object" src="<?=ASSETS?>assets/img/prof/prof1.jpg"
												width="35" height="35" alt="...">
											<i class="busy dot"></i>
											<div class="media-body">
										...
										<li class="media"><img class="media-object" src="<?=ASSETS?>assets/img/prof/prof5.jpg"
												width="35" height="35" alt="...">
											<i class="away dot"></i>
											<div class="media-body">
										...
										<li class="media">
											...
											</div> <img class="media-object" src="<?=ASSETS?>assets/img/prof/prof4.jpg"
												width="35" height="35" alt="...">
											<i class="online dot"></i>
											<div class="media-body">
										...
										<li class="media"><img class="media-object" src="<?=ASSETS?>assets/img/prof/prof2.jpg"
												width="35" height="35" alt="...">
											<i class="busy dot"></i>
											<div class="media-body">
										...
										<li class="media"><img class="media-object" src="<?=ASSETS?>assets/img/prof/prof7.jpg"
												width="35" height="35" alt="...">
											<i class="online dot"></i>
											<div class="media-body">
									...
									<ul class="media-list list-items">
										<li class="media">
											...
											</div> <img class="media-object" src="<?=ASSETS?>assets/img/prof/prof6.jpg"
												width="35" height="35" alt="...">
										...
										<li class="media"><img class="media-object" src="<?=ASSETS?>assets/img/prof/prof8.jpg"
												width="35" height="35" alt="...">
											<i class="offline dot"></i>
											<div class="media-body">
										...
										<li class="media">
											...
											</div> <img class="media-object" src="<?=ASSETS?>assets/img/prof/prof9.jpg"
												width="35" height="35" alt="...">
											<i class="offline dot"></i>
											<div class="media-body">
										...
										<li class="media"><img class="media-object" src="<?=ASSETS?>assets/img/prof/prof10.jpg"
												width="35" height="35" alt="...">
											<i class="offline dot"></i>
											<div class="media-body">
	...
	<script src="<?=ASSETS?>assets/plugins/sparkline/jquery.sparkline.js"></script>
	<script src="<?=ASSETS?>assets/js/pages/sparkline/sparkline-data.js"></script>
	<!-- Common js-->
	...
	function ___CallBack(ROUTE) {
		...
		let StringData = JSON.stringify(JavascriptHook);
		  Swal.fire({
			...
			if (result.value){
				$.ajax({
					type         : 'POST',// define the type of HTTP verb i want to use (POST for our form)
					dataType     : 'JSON',//the data type you are sending to php
					data         : StringData,// our data object
					contentType  : "application/json; charset=utf-8",//the header type, & since we re sending json data then the header should be json too					data         : JSON.stringify(JavascriptHook),// our data object
					url          : '<?=ROOT .'Admin/CollectionAPIs';?>', // the url where we want to POST
					processData  : false,
					encode       : true,//allow json encoding
					crossOrigin  : true,//allow crossdata transmit javascript/ajax/json to php
					async        : true, //allow asynchronous data.. this is actually because of the JSON.STRINGIFY
					crossDomain  : true, 
					headers		 : {
	...
	function ___Callme(BaseId) {
		const JavascriptHook = 
		{"BaseId":BaseId};
		let dataString = JSON.stringify(JavascriptHook);
		Swal.fire({
			title: 'Are you sure?',
			text: "You won't be able to revert this!",
			showCancelButton: true,
        ...
        if (result.value){
            $.ajax({
				...
				url          : '<?=ROOT .'Admin/deleteCategory';?>', // the url where we want to POST
				processData  : false,
				encode       : true,
				crossOrigin  : true,
				async        : true,
