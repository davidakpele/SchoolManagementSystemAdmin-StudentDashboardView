	...
	<div class="wrapper">
		...
		<div class="content-wrapper">
			<section class="content-header">
				<h1>
				Edit Faculty <small>Edit Data</small>
				</h1>
                <div class="box-tools pull-right">
                    <a href="<?=ROOT?>Admin/Faculties" class="btn btn-sm btn-flat btn-primary">
                ...
                <div class="row">
                    <div class="max-width" style="max-width: 540px;margin: 0 auto;background: #FFF;border-radius: 5px;margin: 0 auto;padding: 25px;">
                        <form action="javascript:void(e)" method="post" autocomplete="off">
                            <input type="hidden" value="<?=$data['Fact']->FacultyID?>" id="id" class="form-control">
                            <div class="form-group invalid">	
                            <label for="Application Name" style=":#73879C;">Application Name:</label>
                            <select id="App" class="form-control select2" disabled multiple="multiple">
                                <?php foreach ($data['App'] as $i):?>
                                    <option value="<?=$i['Category__ID']?>" selected="selected" ><?=$i['Category__name']?></option>
                                <?php endforeach;?>
                            </select>
                            </div>
                            <div class="form-group invalid">
                                <label for="Factname">Departement Name: </label>
                                <input type="text" disabled data-id="<?=$data['Fact']->FacultyID?>" value="<?=$data['Fact']->FacultyName?>" id="isEditData" class="form-control">
                                <small class="help-block1" style="color:red"></small>
                            </div>
                        </form>
                        <div class="footer">
                            <button type="button" class="btn btn-secondary btn-sm" id="kv" name="SubmitAppoint" onclick="enable()">Deactivite Disabled</button>
...
<script>
     enable = () =>{
      ...
      isEditData = ()=>{
        ...
        let App = $('#App').val();
        console.log(App);
        let data = {'App':App, 'Depname':name, 'id':id};
        ...
        $.ajax({
            ...
            url: base_url + 'Admin/IsEditFact', // the url where we want to POST
            processData: false,
            encode: true,
            crossOrigin: true,
            async: true,
