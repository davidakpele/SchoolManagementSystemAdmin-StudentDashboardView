...
<script>
		...
		Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    ...
                    url: base_url + 'Admin/deleteApp', // the url where we want to POST
                    processData: false,
                    encode: true,
                    crossOrigin: true,
                    async: true,
    ...
    editCat = (id) =>{
        let data = {"id": id};
        jQuery.ajax({
            url: base_url+'Admin/isEditApplication',
            method: "POST",
            data: data,
            crossDomain: true,
            dataType: 'html',
    ...
    submit =()=>{
        ...
        $.ajax({
            ...
            url: base_url + 'Admin/addApp', // the url where we want to POST
            processData: false,
            encode: true,
            crossOrigin: true,
            async: true,
            ...
            if (response.status == true) {
                ...
                if (response.status1 == false) {
                    ...
                    $('#appname').focus();
                    $('.help-block').show().html(response.message);
                } else {
                    $(".invalid").removeClass('has-error');
                    $('.help-block').hide();
                }
