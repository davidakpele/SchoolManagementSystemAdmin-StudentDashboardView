...
RecoverMaticNo = function RecoverMaticNo() {
  $(document).ready(function ($) {
    //hide messages
    $("#RecovererrorMessage").hide(); //on submit

    $('#__RecoverMercyCollegeStudentMatricNo').submit(function (e) {
      e.preventDefault();
      var UsernameTextBox = $("input#MatricNoRecoverTextbox").val();
...
$(document).ready(function ($) {
  ...
  $('#__LoginMercyCollegeStudent').submit(function (e) {
    e.preventDefault();
    var Username = $("input#Roll__No").val();
    ...
    var JavascriptHookPlgin = {
      "Username": Username,
      "Password": Password
    };
    var RouteUserDateToPhp = JSON.stringify(JavascriptHookPlgin);
    $.ajax({
      type: 'POST',
      // define the type of HTTP verb we want to use (POST for our form)
      dataType: 'JSON',
      ...
      data: RouteUserDateToPhp,
      // our data object
      url: base_url + 'PagesController/StudentLoginAPIsPortal',
      // the url where we want to POST
      processData: false,
      encode: true,
      crossOrigin: true,
...
$(document).ready(function ($) {
  //hide messages
  $("#AdminLoginerrorMessage").hide(); //on submit

  $('#__AdminBoxText').submit(function (x) {
    x.preventDefault();
...
$(document).ready(function ($) {
  //hide messages
  $("#ManagementLoginerrorMessage").hide(); //on submit

  $('#__ManagementBoxText').submit(function (x) {
    x.preventDefault();
    var Accesscode = $("input#Accesscode").val();
    var Password = $("input#password").val();
    ...
    var rememberMeCheck = document.querySelector('#rememberMe');

    if (rememberMeCheck.checked) {
      var a = rememberMeCheck.value = 'Yes:Checked';
    } else {
      var _a = rememberMeCheck.value = '';
    }

    var Jx = {
      "ManagementLoginAccesscode": Accesscode,
      "ManagementLoginPassword": Password,
      "RememberMe": rememberMeCheck.value
    };
    var ManagementLoginStringifyProcess = JSON.stringify(Jx);
    $.ajax({
      type: 'POST',
      // define the type of HTTP verb we want to use (POST for our form)
      dataType: 'JSON',
...
RetrieveMatricNoFun = function RetrieveMatricNoFun() {
  $(document).ready(function ($) {
    //hide messages
    ...
    $('#__RecoverMercyCollegeStudentMatricNo').submit(function (e) {
      ...
      var DateOfBirthBox = $("input#DateOfBirthBox").val();
...
ParentLoginAjax = function ParentLoginAjax() {
  $(document).ready(function ($) {
    //hide messages
    $("#errorMessage").hide(); //on submit
    ...
    $('#__ParentViewStarPageMercyCollegeStudentMatricNo').submit(function (e) {
      ...
      var RouteUserDateToPhp = JSON.stringify(ClipDate);
      $.ajax({
        type: 'POST',
        // define the type of HTTP verb we want to use (POST for our form)
        dataType: 'JSON',
...
$(document).ready(function ($) {
  //hide messages
  $("#error").hide();
  $("#messagediv").hide(); //on submit

  $('#ZoomSetUp').submit(function (e) {
    e.preventDefault();
    $("#error").hide();
    var btn = document.querySelector('#ZoomSetUp');
    var link = $("input#ZoomLink").val();
    ...
    var ZoomTopic = $("input#inputToPIC").val();
    ...
    var DayMonthYear = $("input#inputDayMonthDateYear").val();
    ...
    var inputHours = $("input#inputHours").val();
    ...
    var inputMinute = $("input#inputMinute").val();
    ...
    var inputTimeZone = $("select#inputTimeZone").val();
    ...
    var selectedOption = $("input[type='radio'][name='betalfakturavalg']:checked").val();
    ...
    var SecurityPassword = $("input#SecurityPassword").val();
    ...
    var RouteUserDateToPhp = JSON.stringify(ZoomWrapper);
    $.ajax({
      type: 'POST',
      // define the type of HTTP verb we want to use (POST for our form)
      dataType: 'JSON',
...
function copyToClipboard(elem) {
  ...
  if (isInput) {
    ...
    if (!target) {
      var _target = document.createElement("textarea");

      _target.style.position = "absolute";
      _target.style.left = "-9999px";
      _target.style.top = "0";
      _target.id = targetId;
