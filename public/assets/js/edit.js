...
is_clicked_save_subjectEdit = () => {
	var _id = $('#edit_id').val();
	var _course = $('#course').val();
	var _course_code = $('#coursecode').val();
	var _course_id = $('#courseid').val();
	var _subject_title = $('#subject').val();
	const imsjd = {'id':_id, 'course':_course, 'courseid': _course_id, 'course_code': _course_code, 'subject': _subject_title };
	let data = JSON.stringify(imsjd);
        $.ajax({
            ...
            contentType: "application/json; charset=utf-8",
            data: data, // our data object
            url: base_url + 'Admin/save_edit_subject', // the url where we want to POST
            processData: false,
            encode: true,
            crossOrigin: true,
            async: true,
