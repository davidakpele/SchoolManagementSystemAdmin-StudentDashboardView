$(document).ready(($) => {
    ...
    $('#user_info').submit(function(e) {
        ...
        if (username == "") {
            $(".statusMsg").fadeIn().html("<span style='margin-left:30px'>&nbsp;&nbsp; Enter Username.</span> ");
            $("#username").focus()
            return false;
        }
        if (fname == "") {
            $(".statusMsg").fadeIn().html("<span style='margin-left:30px'>&nbsp;&nbsp; Enter last name</span> ");
            $("#fname").focus()
            return false;
        }
        if (lname == "") {
        ...
        const data = { "id": id, "username": username, "fname": fname, "lname": lname, "email": email };
        ...
        $.ajax({
            ...
            url: base_url+'editusermethod',// the url where we want to POST
            processData: false,
            encode: true,
            crossOrigin: true,
            async: true,
    ...
    $('#isUpdataPassword').submit(function (e) {
        ...
        let confirm = $('#new_confirm').val();
         ...
         $.ajax({
             ...
             url: base_url + 'dassa', // the url where we want to POST
             processData: false,
             encode: true,
             crossOrigin: true,
             async: true,
    ...
    //userlevel
      $('#user_level').submit(function(e) {
        ...
        $.ajax({
            ...
            contentType: "application/json; charset=utf-8",
            data: RouteUserDateToPhp,// our data object
            url: base_url+'edituserlevel',// the url where we want to POST
            processData: false,
            encode: true,
            crossOrigin: true,
            async: true,
