...
$(document).ready(function ($) {
  $("#Application__Type").change(function () {
    ...
    var Url = base_url + 'PagesController/RenderRequirementData'; // the url where we want to POST
...
$(document).ready(function ($) {
  $("#Program__Type").change(function () {
    ...
    var StringData = JSON.stringify(JavascriptHook);
    var Url = base_url + 'PagesController/RenderProgrammeList';
    $.ajax({
      type: 'POST',
      // define the type of HTTP verb we want to use (POST for our form)
      dataType: 'JSON',
      ...
      if (response.Status == 2001) {
        if (val2 == 1) {
          ...
          Response.forEach(function (Fetch) {
            $('#RequirementDiv').append('<h2 class="h-1" style="font-size: 18px">' + Fetch.Child_name + '</h2><br><h2>PROGRAMME OUTLINE:</h2><font size="3"><br /> <br /><h2>DURATION:</h2>' + Fetch.Duration + '<br /> <br /></font> <h3 class="h-1">' + Fetch.headerone + '</h3><br /><font size="2.5">' + Fetch.Subtext + '' + Fetch.UTME);
          });
        } else if (val2 == 2) {
          Response = response.result;
          Response.forEach(function (Fetch) {
            $('#RequirementDiv').append('<h2 class="h-1" style="font-size: 18px">' + Fetch.Child_name + '</h2><br><h2>PROGRAMME OUTLINE:</h2><font size="3"><br /> <br /><h2>DURATION:</h2>' + Fetch.Duration + '<br /><br /></font> <h3 class="h-1">' + Fetch.headerone + '</h3><br /><font size="2.5">' + Fetch.Subtext + '<br /></font><br />' + Fetch.UTME + '');
          });
        } else if (val2 == 3) {
          Response = response.result;
          Response.forEach(function (Fetch) {
            $('#RequirementDiv').append('<h2 class="h-1" style="font-size: 18px">' + Fetch.Child_name + '</h2><br><h2>PROGRAMME OUTLINE:</h2><font size="3"><br /> <br /><h2>DURATION:</h2>' + Fetch.Duration + '<br /> <br /></font> <h3 class="h-1">' + Fetch.headerone + '</h3><br /><font size="2.5">' + Fetch.Subtext + ' <br /></font><br /><h2>UTME</h2>' + Fetch.UTME + '</font><br /><br /><div><b>AND</b></div><br /><h2>WASSCE</h2>' + Fetch.WASSCE + '</font><br /> <br /><div><b>OR</b></div><br /><h2>NECO SSCE</h2>' + Fetch.NECO_SSCE + '<br /><br /><div><b>OR</b></div><br /> <h2>IGCSE</h2>' + Fetch.IGCSE + ' </font><br /><br />');
          });
        }
      }
    }).fail(function (xhr, status, error) {
